...
import SaveUkraine from "../ReusableComponent/SaveUkraine";
...
const OrderListComponent = () => {
    ...
    const getOrderList = async () => {
        setLoading(true)
        try {
            const order = await axios.get(`http://localhost:8081/api/orders`)
            setOrder(order.data)
        } catch (e) {
            setError(e.message)
        } finally {
    ...
    return (
        <Box sx={{ml: 65, maxWidth: '80%', display: 'flex', justifyContent: 'start', alignItems: 'top', mt: 5}}>
            <Box sx={{maxWidth: '80%', width: 825, display: 'flex', flexDirection: 'column'}}>
                <OrderViewList/>
                <Pagination sx={{mt: 3, mb: 3, display: 'flex', justifyContent: 'center'}}
                            count={count} page={pageNumber} onChange={handleChange}/>
            </Box>
            <Box sx={{maxWidth: '20%', width: '20%', ml: 15}}>
                <SaveUkraine/>
            </Box>
        </Box>
    );
