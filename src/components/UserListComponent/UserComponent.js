...
const UserComponent = () => {
    ...
    const getData = async () => {
        setLoading(true)
        try {
            const user = await axios.get(`http://localhost:8081/api/user/${params.id}`);
            setUser(user.data)
    ...
    const deleteUser = (id) => axios.delete(`http://localhost:8081/api/user/${id}`)
        .then(() => navigate('../users', {replace: true}))

    const updateUser = (id) => {
        axios.put(`http://localhost:8081/api/user/${id}`, {
